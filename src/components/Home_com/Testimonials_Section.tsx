...
export default function Testimonials_Section() {
  ...
  const MoveUp = useTransform(scrollYProgress, [0, 1], [2, 0]);
  const Scale = useTransform(scrollYProgress, [0, 1], [0.4, 1]);
  const Opacity = useTransform(scrollYProgress, [0, 1], [0.4, 1]);

  return (
    <div className="h-[5000px] z-[999999] w-full bg-red-600">
      <div className="bg-blue-500 sticky w-full h-[100vh] z-[9999] top-0 flex justify-center items-center">
        {Array.from({ length: 10 }).map(() => (
          <motion.div
            ...
            style={{x:MoveUp}}
            initial="initial"
            className="w-[40%] h-[400px] bg-gray-400 absolute"
          ></motion.div>
        ))}
      </div>
    </div>
